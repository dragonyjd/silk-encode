...
int silk_encoder_main( int argc, char* argv[] )
{
    ...
    SKP_int32 targetRate_bps = 16000;
...
SKP_int32 targetRate_bps = 16000;
SKP_int32 packetSize_ms = 20, totPackets = 0;
SKP_int32 frameSizeReadFromFile_ms = 20;
SKP_int32 packetLoss_perc = 0;
#if LOW_COMPLEXITY_ONLY
SKP_int32 complexity_mode = 0;
...
int Silk_Encoder_Encode(SKP_int16 * inBuf, int inLen, unsigned char * outBuf, int outLen)
{
    if( ( SKP_int )inLen < ( ( frameSizeReadFromFile_ms * API_fs_Hz ) / 1000 ) ) {
        return 0;
    }
    if (psEnc == NULL) {
        Silk_Encoder_Init();
    ...
    /* max payload size */
    SKP_int16 nBytes = outLen;


    /* Silk Encoder */
    SKP_int ret = SKP_Silk_SDK_Encode( psEnc, &encControl, inBuf, (SKP_int16)inLen, outBuf, &nBytes );
    if( ret ) {
        LOGD( "\nSKP_Silk_Encode returned %d", ret );
    }
    totPackets++;
...
double Silk_Encoder_Finish()
{
    ...
    return totPackets * 1e-3 * packetSize_ms;
}

//////////////////////////decode////////////////////////////
extern void decode_callback(short outBuf[], uint32_t outLen);
bool g_decoder_abort = false;

// 增大音量
void IncreaseVolume(short outBuf[], uint32_t outLen) {
    int32_t pcmval;
    for (int ctr = 0; ctr < outLen; ctr++) {
        pcmval = outBuf[ctr] * 2;
...
int Silk_Decoder(const char* filename)
{
    ...
    while( 1 ) {
        ...
        if( lost == 0 ) {
            ...
            do {
                ...
                /* Until last 20 ms frame of packet has been decoded */
            } while( DecControl.moreInternalDecoderFrames && !g_decoder_abort);
        } else {
            /* Loss: Decode enough frames to cover one packet duration */
            for( i = 0; i < DecControl.framesPerPacket && !g_decoder_abort; i++ ) {
                /* Generate 20 ms */
                ret = SKP_Silk_SDK_Decode( psDec, &DecControl, 1, payloadToDec, nBytes, outPtr, &len );
                if( ret ) {
                    LOGD( "\nSKP_Silk_Decode returned %d", ret );
        ...
        if (!g_decoder_abort) {
            //fwrite(out, sizeof(SKP_int16), tot_len, speechOutFile);
            IncreaseVolume(out, tot_len);
            decode_callback(out, tot_len);
        }
    ...
    /* Empty the recieve buffer */
    for( k = 0; k < MAX_LBRR_DELAY && !g_decoder_abort; k++ ) {
        if( nBytesPerPacket[ 0 ] == 0 ) {
            /* Indicate lost packet */
            lost = 1;
            ...
            /* Packet loss. Search after FEC in next packets. Should be done in the jitter buffer */
            payloadPtr = payload;
            for( i = 0; i < MAX_LBRR_DELAY && !g_decoder_abort; i++ ) {
                if( nBytesPerPacket[ i + 1 ] > 0 ) {
                    starttime = GetHighResolutionTime();
                    SKP_Silk_SDK_search_for_LBRR( payloadPtr, nBytesPerPacket[ i + 1 ], ( i + 1 ), FECpayload, &nBytesFEC );
                    tottime += GetHighResolutionTime() - starttime;
